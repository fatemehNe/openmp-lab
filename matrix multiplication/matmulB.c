...
void multiply(DataSet dataSet){
	int i, j, k, sum;
	omp_set_nested(1);
	#pragma omp parallel private(j,k,sum) num_threads(1)
	{
		//printf("%d thread", omp_get_num_threads());
		#pragma omp for schedule(static, 320)
		for (i = 0; i < dataSet.n; i++) {
			#pragma omp parallel  private(j,k,sum) num_threads(1)
			{
				//printf("%d num", omp_get_num_threads());
				#pragma omp for schedule(static, 320)
				for (j = 0; j < dataSet.p; j++) {
					sum = 0;
